...
import com.trader.stringdata.Strings.ExploreLinks;
...
public class HomeHelper extends DriverHelper {
	...
	public void assertTopMakesSection() {
		print("Top Trader Makes Section");
	...
	public void clickAndAssertPopularTypeLinks() {
		...
		for (String popularTypeList : allpopularTypeLists) {
			...
			clickOn(locator);
			String typeTitle = "//*[@id='topicTitle']";
			String txt = getText(typeTitle);
			System.out.println(txt);
			Assert.assertTrue(txt.contains(popularTypeList));
			this.clickOnApplicationLogo();

		}
	}
	
	public void clickOnStateIconAndAssertResultPage() {
		print("Click On popular State Icon And Assert Result Page.");
		int iconCount = getWebDriver().findElements(ByLocator(Locator.State.Popular_State_Icon.value)).size();
		
		for(int i=1; i<iconCount;i++) {
			String icon = "//*[@id='statesIcons']/li["+i+"]//span";
			String iconTxt = getText(icon).toLowerCase();
			print(iconTxt);
			String stateIcon = "//*[@id='statesIcons']/li["+i+"]//a";
			clickOn(stateIcon);
			waitForElementPresentInDom(1);
			String resultPage = getText(Locator.Search.Ad_SearchResult.value).toLowerCase();
			print(resultPage);
			Assert.assertTrue(resultPage.contains(iconTxt));
			this.clickOnApplicationLogo();
	...
	/**
	 * ==================================================================================
	 * Click On Popular Types Icons And Assert Result Page.
	 * ==================================================================================
	 */
	public void clickOnPopularTypesIconsAndAssertResultPage() {
		print("Click On Popular Types Icons And Assert Result Page.");
		int iconCount = getWebDriver().findElements(ByLocator(Locator.Types.Popular_Types_Icon.value)).size();
		for(int i=1; i<iconCount;i++) {
			String typeName = "//*[@id='typeIcons']/li["+ i + "]/span";
			String typeNameTxt = getText(typeName);
			print(typeNameTxt);
			waitForElementPresentInDom(1);
			String typeIcon = "//*[@id='typeIcons']/li["+ i + "]/a";
			clickOn(typeIcon);
			waitForElementPresentInDom(1);
			clickOn(Locator.Home.Find_Button.value);
			waitForElementPresentInDom(1);
			String resultPage = getText(Locator.Search.Ad_SearchResult.value);
			print(resultPage);
			Assert.assertTrue(resultPage.contains(typeNameTxt));
			this.clickOnApplicationLogo();
			this.clickOnPopularTypesTab();
		}
	}
	...
	public void clickAndAssertTypesLink() {
		...
		int length = getWebDriver()
				.findElements(By.xpath("//*[@id='wrapper']//div[3]/ul[@class='list-unstyled']/li")).size();
		System.out.println("read loactor");

		System.out.println(length);
		for (int i = 1; i <= length; i++) {

			locator = "//*[@id='wrapper']//div[3]/ul[@class='list-unstyled']/li[" + i + "]/a";
	...
	public void clickAndAssertPopularStateLinks() {
		...
		for (String popularStateList : allpopularStateLists) {
			...
			clickOn(locator);
			String stateTitle = "//*[@id='topicTitle']";
			String txt = getText(stateTitle);
			System.out.println(txt);
			Assert.assertTrue(txt.contains(popularStateList));
	...
	public void clickAndAssertStateLinks() {
		...
		for (int i = 1; i <= length; i++) {
			...
			locator = "//div[@class='browseContainer']/div[" + i + "]//a";
			String text = getText(locator).replaceAll("[()0-9, ]", "").toLowerCase();
//			String[] text2 = text.split(" ");
			...
			this.clickOnApplicationLogo();
			this.clickOnPopularStateTab();
			this.clickOnSearchAllUnderState();
			this.assertStateResultPage();
		}
	}
	...
	public List<String> getExploreLinks() {
		EnumSet<ExploreLinks> all = EnumSet.allOf(ExploreLinks.class);
		List<String> eLinks = new ArrayList<>(all.size());
		for (ExploreLinks s : all) {
			eLinks.add(s.value);
		}
		return eLinks;
	}
	...
	/**
	 * Assert All links and mapped url's for all links under Explore Cycle Trader section.
	 */
	public void clickOnExploreLinksAndAssertResultPage() {
		print("Assert All links and mapped url's for all links under Explore Cycle Trader section.");
		
		String aboutLocator = "//*[contains(@title,'About')]";
		String text = getWebDriver().findElement(ByLocator(aboutLocator)).getAttribute("href");
		System.out.println(text);
		Assert.assertTrue(text.contains(Strings.ExploreLinks.About.value));
		
		String riderfuel = "//*[contains(@title,'Rider Fuel')]";
		text = getWebDriver().findElement(ByLocator(riderfuel)).getAttribute("href");
		System.out.println(text);
		Assert.assertTrue(text.contains(Strings.ExploreLinks.Rider_Fuel_Resources.value));
		
		String blog = "//*[contains(@title,'Blog')]";
		text = getWebDriver().findElement(ByLocator(blog)).getAttribute("href");
		System.out.println(text);
		Assert.assertTrue(text.contains(Strings.ExploreLinks.Blog.value));
		
		String raceTeam = "//*[contains(@title,'.com Race Team')]";
		text = getWebDriver().findElement(ByLocator(raceTeam)).getAttribute("href");
		System.out.println(text);
		Assert.assertTrue(text.contains(Strings.ExploreLinks.Race_Team.value));
		
		String siteMap = "//*[contains(@title,'Sitemap')]";
		text = getWebDriver().findElement(ByLocator(siteMap)).getAttribute("href");
		System.out.println(text);
		Assert.assertTrue(text.contains(Strings.ExploreLinks.SiteMap.value));
		
		String help = "//*[contains(@title,'Need Help')]";
		text = getWebDriver().findElement(ByLocator(help)).getAttribute("href");
		System.out.println(text);
		Assert.assertTrue(text.contains(Strings.ExploreLinks.Need_Help.value));

		// int length =
		// getWebDriver().findElements(By.xpath("(//*[contains(@class,'footerList')])[1]/li")).size();
		// System.out.println(length);
		// List<String> exploreCycleTraderLinks = getExploreLinks();
		// for(int i = 1; i>=length; i++) {
		// for (String linksTxt : exploreCycleTraderLinks) {
		// waitForElementPresentInDom(1);
		// locator = "(//*[contains(@class,'footerList')])[1]/li[" + i + "]/a";
		// String text =
		// getWebDriver().findElement(ByLocator(locator)).getAttribute("href");
		// System.out.println(text);
		// Assert.assertTrue(text.contains(linksTxt));
		// }}
	}
	...
	public void assertEmailUSButtonLinkUnderGiveUsFeedback() {
		...
		waitForElementPresentInDom(5);
		String string = getWebDriver().findElement(ByLocator(Locator.Home.Email_Us_Feedback.value))
				.getAttribute("href");
	 ...
	 * @throws AWTException
	 */
	public void clickOnDealerLogin_SignUpButtonAndAssertResultPage() {
		...
		String url = getWebDriver().findElement(ByLocator(dealerLogin_SignUp)).getAttribute("href");
		Assert.assertEquals(url, "https://dealers.cycletrader.com/");
	}
	 ...
	 * @throws AWTException
	 */
	public void clickOnDealerSearchButtonAndAssertResultPage() {
	...
	public void clickFooterLinksAndAssertResultPage(){
		print("Assert all footer link configured url.");
		String securityCentre =  Locator.footerLinks.Security_Center.value;
		String linkURL = getWebDriver().findElement(ByLocator(securityCentre)).getAttribute("href");
		Assert.assertTrue(linkURL.contains(Strings.FooterLinksURL.Security_Center.value));
		
		String advertiserAgreement =  Locator.footerLinks.Advertiser_Agreement.value;
		linkURL = getWebDriver().findElement(ByLocator(advertiserAgreement)).getAttribute("href");
		Assert.assertTrue(linkURL.contains(Strings.FooterLinksURL.Advertiser_Agreement.value));
		
		String CommunityGuidelines =  Locator.footerLinks.Community_Guidelines.value;
		linkURL = getWebDriver().findElement(ByLocator(CommunityGuidelines)).getAttribute("href");
		Assert.assertTrue(linkURL.contains(Strings.FooterLinksURL.Community_Guidelines.value));
		
		String copyright =  Locator.footerLinks.Copyright.value;
		linkURL = getWebDriver().findElement(ByLocator(copyright)).getAttribute("href");
		Assert.assertTrue(linkURL.contains(Strings.FooterLinksURL.Copyright.value));
		
		String privacy =  Locator.footerLinks.Privacy_Policy.value;
		linkURL = getWebDriver().findElement(ByLocator(privacy)).getAttribute("href");
		Assert.assertTrue(linkURL.contains(Strings.FooterLinksURL.Privacy_Policy.value));
		
		String terms =  Locator.footerLinks.Terms_of_Use.value;
		linkURL = getWebDriver().findElement(ByLocator(terms)).getAttribute("href");
		Assert.assertTrue(linkURL.contains(Strings.FooterLinksURL.Terms_Use.value));
	}
